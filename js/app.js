...
// ui te data sorasori load er jonno function call korte hoy
loadData();
    ...
    div.innerHTML = `
       ...
       <div style="min-height:450px" class="card h-100">
           ...
           <div class="card-body text-dark text-center">
               <h5 class="card-title text-info">${product.title.slice(
                 0,
                 20
               )}</h5>
               <h4 class="text-center">Price: $ ${product.price}</h4>
               <p class="card-text text-capitalize"><b>Category: ${
                 product.category
               }</b></p>
               <p class="card-text">
                   <b>Ratings:
                   <span>${showRating(product.rating.rate)}</span> <span>${
      product.rating.rate
    }</span>
                   </b>
               </p>
           ...
           <div class="card-footer d-flex justify-content-around">
               ...
               <button onclick='details(${
                 product.id
               })' type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal">Details</button>
           </div>
       </div>
   </div>
 `;
...
function showRating(rating) {
  const rateIcon = Math.floor(rating);
...
function details(id) {
  ...
  fetch(api)
    .then((res) => res.json())
    .then((product) => singleDetail(product));
}

function singleDetail(product) {
...
// toggle spinner
function loadSpinner(displayStyle) {
